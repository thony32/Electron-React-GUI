...
const MainContextMenu: React.FC<ContextMenuProps> = ({ top, left }) => {
    ...
    return (
        ...
        <span className="text-xs font-semibold">{OS(window) === "MacOS" ? "Cmd + H" : "Ctrl + H"}</span>
            ...
            <button className="py-2 px-4 text-sm hover:bg-base-200 duration-300 rounded-sm flex justify-between items-center space-x-4" onClick={handleUndo} disabled={!canUndoNodes && !canUndoEdges}>
                <span className="font-bold tracking-wide">Undo</span>
                <span className="text-xs font-semibold">{OS(window) === "MacOS" ? "Cmd + Z" : "Ctrl + Z"}</span>
            </button>
            <button className="py-2 px-4 text-sm hover:bg-base-200 duration-300 rounded-sm flex justify-between items-center space-x-4" onClick={handleRedo} disabled={!canRedoNodes && !canRedoEdges}>
