...
const useBrush = (image: HTMLImageElement, setImageSrc: (src: string) => void, width: number, height: number) => {
  ...
  const originalCanvas = useRef(document.createElement("canvas")) as MutableRefObject<HTMLCanvasElement>;
  ...
  const isInColorRange = (targetColor: Uint8ClampedArray, compareTo: Uint8ClampedArray, i: number) => targetColor[i] >= compareTo[0] - 10 && targetColor[i] <= compareTo[0] + 10 && targetColor[i + 1] >= compareTo[1] - 10 && targetColor[i + 1] <= compareTo[1] + 10 && targetColor[i + 2] >= compareTo[2] - 10 && targetColor[i + 2] <= compareTo[2] + 10;

  const isInSpaceRange = (image: HTMLImageElement, i: number, x: number, y: number, width: number, height: number) => i >= y - width - image.width * height && i <= y + width + image.width * height && i % (image.width * 4) >= x - width && i % (image.width * 4) <= x + width;

  const selectColor = (colorData: Uint8ClampedArray, x: number, y: number, width: number, height: number) => {
    const canvasData = canvas.current.getContext("2d")!.getImageData(0, 0, canvas.current.width, canvas.current.height);
  ...
  const removeColor = (colorData: Uint8ClampedArray, x: number, y: number, width: number, height: number) => {
    const canvasData = canvas.current.getContext("2d")!.getImageData(0, 0, canvas.current.width, canvas.current.height);
    const pix = canvasData.data;
    for (let i = 0, n = pix.length; i < n; i += 4) {
      if (isInColorRange(pix, colorData, i)) {
  ...
  const healColor = (position: { x: number; y: number }) => {
    canvas.current.getContext("2d")!.clearRect(0, 0, image.width, image.height);
    canvas.current.getContext("2d")!.drawImage(originalImage.current, 0, 0, originalImage.current.width, originalImage.current.height);
  ...
  const detectColor = (position: { x: number; y: number }) => {
    const colorData = canvas.current.getContext("2d")!.getImageData(position.x, position.y, 1, 1).data;
    return colorData;
  };
